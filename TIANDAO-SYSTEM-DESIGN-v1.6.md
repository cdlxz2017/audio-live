...
**版本定位：** 角色体系重构 v2.0——支持动态角色创建 + 部门归属体系
...
CREATE TABLE department (
    ...
    parent_id       UUID REFERENCES department(id) COMMENT '上级部门（层级结构）',
    realm           VARCHAR(20) NOT NULL COMMENT '所属领域：天庭/地府/人间/跨三界',
    description     TEXT,
    manager_role_id UUID REFERENCES role(id) COMMENT '部门负责人角色',
...
COMMENT ON TABLE department IS '部门表，支持天庭/地府/人间三级部门体系';
```

### role 表新增 department_id
...
```sql
INSERT INTO department (name, code, realm, description) VALUES
-- 天庭部门
('天枢部', 'TIANHEAVEN_01', '天庭', '星辰运转与天机管理'),
('雷部', 'TIANHEAVEN_02', '天庭', '雷电风雨气象事务'),
('水部', 'TIANHEAVEN_03', '天庭', '江河湖海水系管理'),
('功德司', 'TIANHEAVEN_04', '天庭', '记录三界功德业障'),
('考功司', 'TIANHEAVEN_05', '天庭', '考核神明政绩'),
-- 地府部门
('第一殿', 'UNDERWORLD_01', '地府', '秦广王殿'),
('轮回司', 'UNDERWORLD_06', '地府', '轮回投胎事务'),
-- 人间部门
('人间道', 'MORTAL_00', '人间', '凡人修行管理'),
-- 跨三界
('天道秘书处', 'TIANDAO_ADMIN', '跨三界', '天道最高秘书机构');
```
...
_本方案 v1.6：角色动态化（支持管理员新建角色）+ 部门归属体系（所有角色必须归属部门）+ 初始8个部门数据。_
