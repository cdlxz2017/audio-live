...
('HEAVEN_EMPEROR', '天帝', '天庭最高统治者，主管天庭一切事务', TRUE),
('HALL_MASTER', '殿主', '地府十殿殿主，管理本殿一切事务', TRUE),
('JUDGE', '判官', '负责判决魂魄、记录功德业障', TRUE),
('REINCARNATION_MASTER', '轮回司主', '负责轮回投胎事务', TRUE),
('LIGHTNING_GOD', '雷部天神', '主管雷电、劫难事务', TRUE),
('WATER_GOD', '水部天神', '主管降雨、水患事务', TRUE),
...
WHERE r.code = 'HEAVEN_EMPEROR'
  AND p.category IN ('member', 'heaven');
...
WHERE r.code = 'WATER_GOD'
  AND p.code IN ('heaven:weather:control','heaven:disaster:read',
                 'member:read','notice:read');
...
_本方案 v1.4：LIGHTNING_GOD 权限修正为显式列举；新增权限继承规范；新增迁移说明指南。_
